...
- 통합 업데이트 알림. 앱 실행 시 앱 자체·UE4SS 런타임·설치된 모드의 업데이트를 병렬로 확인하고(각 확인은 실패가 서로 격리됨), 업데이트가 있으면 모드 탭 우하단에 지속형 카드로 표시한다. 모드와 UE4SS 런타임은 카드에서 바로 업데이트하고, 앱 업데이트는 자동 설치 대신 GitHub 릴리즈 페이지를 연다(앱이 ad-hoc 서명이라 자동 설치 불가). 사이드바에서 수동 재확인 가능.

### 변경

- 모드 리스트의 행별 "업데이트" 배지 제거 — 업데이트 안내는 통합 패널로 일원화.
- Play 화면: 배경 영상 2종 추가, 가독성 오버레이를 완화해 어두운 영상이 과하게 눌리지 않도록 조정.
